    ...
    <div className="flex h-full flex-col bg-gray-700">
      ...
      <nav className="no-scrollbar flex-1 overflow-y-auto px-3 py-5" aria-label="Admin navigation">
        {navGroups.map((group, gi) => {
          ...
          return (
            <div
              key={group.label}
              className={`${gi > 0 ? 'mb-2' : 'mb-3'} ${
                isExpandable ? `rounded-lg bg-gray-800 ${collapsed ? 'p-1.5' : 'p-2.5'}` : ''
              }`}
            >
              {/* Section header (expandable groups only) */}
              {!collapsed && isExpandable && (
                <button
                  onClick={() => toggleGroup(group.label)}
                  className="mb-1 flex w-full items-center justify-between px-2.5 py-1.5 text-xs font-bold uppercase tracking-[0.15em] text-[#9CA3AF] transition-colors hover:text-slate-200"
              ...
              {/* Nav items */}
              {(!group.expandable || isGroupExpanded || collapsed) && (
      ...
      <div className="shrink-0 border-t border-white/[0.08] p-3">
        <button
          onClick={onLogout}
          disabled={loggingOut}
          className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-400 transition-all duration-150 hover:bg-red-500/10 hover:text-red-400 disabled:opacity-50 ${
            collapsed ? 'justify-center px-0' : ''
          }`}
        >
