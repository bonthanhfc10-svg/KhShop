...
export default function EditSupplier() {
  ...
  useEffect(() => {
    supplierService.getById(id).then((supplier) => {
      if (!supplier) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate('/admin/suppliers', { state: { toast: 'Updated successfully' } });
    } catch (err) {
  ...
  if (error && !form) {
    return (
      <div className="space-y-5">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Edit Supplier</h1>
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Edit Supplier</h1>
          <p className="mt-1 text-sm text-slate-500">Update supplier information.</p>
        </div>
      </div>
